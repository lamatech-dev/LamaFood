...
class BusinessContextController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
        abort_if($user->business === null, 422, 'Choose a business context before managing content.');
        $business = $user->business->load(['locales' => fn ($query) => $query->where('is_enabled', true)->orderByDesc('is_default')->orderBy('id'), 'branches' => fn ($query) => $query->where('is_active', true)->orderByDesc('is_default')]);
